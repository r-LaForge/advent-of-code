def checksum_clear(i):
    """
    Compute the sum of the largest difference in each iterable. (each tuple was a line in the spreadsheet)
    :param i: (int[][]) -> A list of iterables where each iterable is a 'row'.
    :return: The checksum (sum of the largest difference in each iterable)
    """
    total = 0
    for row in i:
        smallest = min(row)
...
def checksum_concise(i):
    """
    See above. This is simply a shorter version.
    """
    return sum([diff for diff in [max(row) - min(row) for row in i]])
...
if __name__ == '__main__':
    ...
    print checksum_clear(formatted_rows)

    print checksum_concise(formatted_rows)
