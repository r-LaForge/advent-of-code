import {calculateFuelForModuleMassesAndFuel} from './calculate-fuel';
...
describe('calculateFuelForModuleMassesAndFuel', () => {
  test('should return 0 if given an empty array', () => {
    expect(calculateFuelForModuleMassesAndFuel([])).toBe(0);
  });
  ...
  describe('for a single module mass', () => {
    ...
    testCases.forEach(t => {
      it('should correctly compute the required fuel', () => {
        try {
          expect(calculateFuelForModuleMassesAndFuel([t.mass])).toEqual(t.expected);
        } catch (e) {
  ...
  describe('for multiple masses', () => {
    it('should compute 51316 fuel when given modules with masses 12, 14, 1969, 100756', () => {
      expect(calculateFuelForModuleMassesAndFuel([12, 14, 1969, 100756])).toEqual(51316);
    });
  });

  test('determine advent input', () => {
    const answer = calculateFuelForModuleMassesAndFuel(ADVENT_DAY_2_PART_2_INPUT);
